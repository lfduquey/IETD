...
#' @export
AAEA<- function(Time_series,format,MaxIETD=maxI
                ,xlabel=paste0("IETD",sep= " [",as.character(format),"]"),ylabel="AAE") {
  if(format=="mins"){maxI=60*3
  }else if (format=="hours"){maxI=24
  }else if (format =="day"){maxI=3}
  else {print ("Choose time format in mins, hours or day")}
